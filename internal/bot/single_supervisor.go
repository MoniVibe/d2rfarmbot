...
func (s *SinglePlayerSupervisor) Start() error {
	...
	for {
		...
		case <-ctx.Done():
			return nil
		default:
			// In attach mode we're already in a game, so skip the char-select wait
			// (it would never resolve and would stall the supervisor).
			if firstRun && !s.bot.ctx.Manager.InGame() {
