...
// HoldKey holds a raw key down for the given duration then releases it. Needed for
// hold-to-act bindings like Force Move, where a quick tap doesn't span a movement frame.
func (hid *HID) HoldKey(key byte, hold time.Duration) {
	win.PostMessage(hid.gr.HWND, win.WM_KEYDOWN, uintptr(key), hid.calculatelParam(key, true))
	time.Sleep(hold)
	win.PostMessage(hid.gr.HWND, win.WM_KEYUP, uintptr(key), hid.calculatelParam(key, false))
}
